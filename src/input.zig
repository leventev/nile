...
const KeyEventBuffer = struct {
    buffer: [buffer_event_count]KeyEvent,
    read_index: usize,
    write_index: usize,
    written: usize,

    const buffer_event_count = 16;
};

// TODO: locking
var event_buffer: KeyEventBuffer = undefined;

pub fn addKeyEvent(event: KeyEvent) void {
    if (event_buffer.written == KeyEventBuffer.buffer_event_count)
        return;
...
pub fn readKeyEvent() ?KeyEvent {
    if (event_buffer.written == 0)
        return null;

    const ev = event_buffer.buffer[event_buffer.read_index];

    event_buffer.read_index = event_buffer.read_index % KeyEventBuffer.buffer_event_count;
    event_buffer.written -= 1;
