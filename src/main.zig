...
pub fn panic(
    ...
    while (frame_pointer != 0) {
        ...
        const next_base = frame_pointer - 2 * @sizeOf(usize);
        if (!std.mem.Alignment.of(usize).check(next_base))
            break;
        const base: [*]usize = @ptrFromInt(next_base);
...
pub fn init(root_page_table: arch.PageTable, dt_ptr_virt: *void) noreturn {
    ...
    mm.init(static_mem_allocator, &dt, root_page_table);
